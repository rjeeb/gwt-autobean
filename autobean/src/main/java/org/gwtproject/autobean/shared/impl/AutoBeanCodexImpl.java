...
public class AutoBeanCodexImpl {
  ...
  /**
   * Recurses into {@link org.gwtproject.autobean.shared.impl.AutoBeanCodexImpl}.
   */
  ...
  public static <T> AutoBean<T> doDecode(EncodeState state, Class<T> clazz, Splittable data) {
    ...
    AutoBean<T> toReturn = (AutoBean<T>) data.getReified(org.gwtproject.autobean.shared.impl.AutoBeanCodexImpl.class.getName());
    if (toReturn != null) {
      return toReturn;
    }
    toReturn = state.factory.create(clazz);
    data.setReified(org.gwtproject.autobean.shared.impl.AutoBeanCodexImpl.class.getName(), toReturn);
    if (toReturn == null) {
